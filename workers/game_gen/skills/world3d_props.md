...
78 themed kinds with named `variant`s (metres), instanced like every kind: `scatter(kind, n, {variant, area, avoid, colors, scale, lights})`, `place(kind, [[x,z] | {x,z,rot,s}], o)`, `make(kind, {variant, pos})`.
...
`name` ∈ `industrialNight harbour village scifiBase ruins city`; `o = {seed, radius (half−6), density=1, edge, landmark}`. A perimeter wall with a gate on the far (−z) side, a straight avenue
