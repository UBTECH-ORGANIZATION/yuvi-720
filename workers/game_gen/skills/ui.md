...
| `YuviUI.ask({parts|text, kind, correct, …})` | question panel (bottom-centre overlay, or inside `el`). `kind`: `"number"` (default; 1 box, `inputmode=decimal`), `"text"`, `"choice"` (`answers:[…]`, `correct` = index or text, `shuffle:true` deterministic), `"ratio"` (two boxes `a : b`, LTR order; answer `"a:b"`; `reduce:true` accepts equivalent ratios), `"pair"` (two labelled numbers, `labels:["x","y"]`, answer `"a,b"`, `correct:"3,4"` or `[3,4]`). Options: `givens: [parts, parts…]` (the numbers the kid works from — gross, tare, coordinates — rendered as rows inside the panel; REQUIRED whenever the answer is computed from data, because a 3D sign or a HUD box elsewhere is not visible while the panel is open), `hint`, `placeholder`, `submitLabel`, `explain` (parts shown ~2 s on a wrong answer), `timeout` (seconds), `el`. Numbers grade tolerantly (`"12"`, `"12.0"`, `"12,0"`). Enter submits; wrong → shake + explain | `Promise<{correct, answer, elapsed}>` (+ `timedOut` / `cancelled`) |
