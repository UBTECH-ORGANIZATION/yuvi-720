...
| `W.player.avatar(char, o)` | `o = {speed=6, run=1.6 (Shift), jump=7, gravity=22, camera:'follow'\|'shoulder'\|'iso'\|'top', distance, height, lag=.12, turn=10, bounds, obstacles, pos, face:[x,y,z] (a point to start facing) \| yaw, sensitivity}` — the hero spawns facing -z (into the screen), so the goal at a smaller z than `pos` is in view; pass `face` (or call `ctrl.lookAt`) when it is elsewhere — WASD/arrows/joystick relative to the camera, turns toward the movement, jumps on Space, follows the terrain, drives `char.anim`; mouse look orbits `follow`/`shoulder` after `ctrl.requestLook()` | `ctrl = {object, pos, vel, yaw, camYaw, onGround, moving, running, update(dt), enable(b), collide(handle\|dec\|positions), teleport(v), lookAt(v), requestLook(), forward(), eye()}` |
