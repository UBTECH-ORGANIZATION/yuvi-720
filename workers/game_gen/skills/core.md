...
The HUD bar already has ⏸ pause, 🔊 mute and ❔ "how to play" (it re-shows your `controls` list) — never add your own pause/help/sound buttons; give `controls` every key and mouse action the game uses.
...
YuviKit.safe() → {top, bottom, left, right} px   // the band the HUD bar and docked panels occupy: canvas-drawn cards, words, targets and enemies stay inside the remaining rectangle (a card painted at y < safe().top sits under the HUD)
