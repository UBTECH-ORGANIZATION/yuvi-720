...
- First frame fast: build the whole scene (geometry, materials, lights, props) while the title screen shows, so `onStart` only flips state and the first frame is drawn within one second of the Start click — the checker fails a canvas that stays flat for 7 s. Keep 3D cheap for software rendering at 640×360: instanced or merged props, one shadow-casting light at most, no post-processing, no per-frame allocations.
- Every DOM lookup is guarded (`const el = document.getElementById(...); if (el) …`) and every event handler tolerates a missing element — a null `.gate` or `.hud` on the first frame is the most common crash the checker catches.
