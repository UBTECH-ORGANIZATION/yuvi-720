 ...
 * to its literal before/after documents.
 */

import { useEffect, useState } from 'react'
import { useI18n } from '../i18n/I18nProvider'
import { listAudit, type AuditEntry, type Person } from './api'
import type { AdminData } from './ConsoleDashboard'
import { formatDayTime } from './dates'
import { EmptyState, Icon, Panel } from './primitives'
import { AdminSection, nameOf, useUnauthorizedGuard } from './shared'

export function AuditTab({ data }: { data: AdminData }) {
  const { t } = useI18n()
  ...
  useEffect(() => {
    ...
    listAudit({
      ...
      .catch((error: unknown) => { if (active && !unauthorized(error)) setEntries([]) })
      .finally(() => { if (active) setIsLoading(false) })
    return () => { active = false }
  }, [actorId, targetId, unauthorized])
      ...
      <AdminSection title={t('adm.audit.title')} hint={t('adm.audit.hint')}>
        <div className="adm-filters">
          <label className="adm-search">
            <span className="adm-sr-only">{t('adm.audit.byActor')}</span>
            <select className="adm-input" value={actorId}
          ...
          <label className="adm-search">
            ...
            <span className="adm-sr-only">{t('adm.audit.byTarget')}</span>
            <input
              type="search"
              className="adm-input"
              value={targetId}
    ...
    <li className="adm-auditRow">
      ...
      {entry.before || entry.after ? (
        <>
          ...
          >
            <Icon name={open ? 'chevronUp' : 'chevronLeft'} size={13} className="adm-icon--flip" />
            {t('adm.audit.diff')}
          </button>
          {open ? (
            <Panel className="adm-auditRow__diff">
              <div>
                ...
                <pre dir="ltr">{JSON.stringify(entry.before, null, 2) ?? '—'}</pre>
              </div>
              <div>
                <h4>{t('adm.audit.after')}</h4>
                <pre dir="ltr">{JSON.stringify(entry.after, null, 2) ?? '—'}</pre>
              </div>
            </Panel>
          ) : null}
        </>
