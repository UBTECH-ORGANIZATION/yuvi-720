...
tools: [vscode, execute, read, agent, edit, search, web, browser, 'azure-devops-yuvi/*', azure-mcp/search, todo]
...
- Load the `720-lrs-reporting` skill before implementing, reviewing, or debugging outbound xAPI reporting to the MoE LRS (720): transport/OAuth, the statement envelope (exidentifier/grouping/team), the 14 event families and their trigger wiring, the `lrs_outbox` retry/dedup ledger, or the Kata (CET) catalog/launcher/relay integration. Implementation lives in `backend/app/services/lrs/`; routes call only `reporter.report_*`.
