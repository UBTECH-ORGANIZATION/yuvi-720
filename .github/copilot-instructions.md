...
- **Design for microservices from day one.** Keep bounded contexts (brain/context engine, xAPI ingest, agents orchestrator, insights, mentoring, org) as separable modules with no shared in-memory state, and treat the event bus and the proactive push channel as interfaces — so the current App Service monolith can later split into a **small set of AKS services (spark-web · brain-agents · xapi-events) wired by Azure Service Bus** with no agent/brain contract change. Keep the service count low; insights/mentoring/org stay modules until a real bottleneck appears. See `docs/architecture/shared-learning-brain.md` §16.
