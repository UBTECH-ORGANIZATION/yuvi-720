...
const dx=async()=>p.evaluate(()=>{const a=document.querySelector('.landing720-yubi-artwork').getBoundingClientRect(),q=document.querySelector('.landing720-yubi-pilot').getBoundingClientRect();return Math.round(Math.abs((a.left+a.width/2)-(q.left+q.width/2)))})
