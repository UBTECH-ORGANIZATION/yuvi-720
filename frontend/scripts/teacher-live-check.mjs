...
try {
  ...
  // The per-student presence surface is the live view on the students screen
  // (#249) — Home keeps the alert inbox, which the handoff block below still
  // exercises there.
  ...
  check('the live view renders on the students screen', true)

  const rows = await teacher.locator('.tch-liveRow').count()
  check('the view lists the whole group, absent included', rows === 12, `${rows} rows`)

  // "Never a bare dot": every row carries a last-seen label beside its dot.
  ...
  await teacher.screenshot({ path: `${OUT}/01-live-view.png`, fullPage: true })
  ...
  check('a student signing in turns their row live, with no reload',
        elapsed >= 0, elapsed >= 0 ? `${elapsed}ms` : 'timed out')
  await teacher.screenshot({ path: `${OUT}/02-student-online.png`, fullPage: true })

  // The alert flow below lives on Home's attention inbox.
  await teacher.goto(`${BASE}/teacher`, { waitUntil: 'domcontentloaded' })
  await teacher.waitForSelector('.tch-home', { timeout: 30000 })
  ...
  await outsider.waitForSelector('.tch-home', { timeout: 40000 })
  ...
  // ── the live view carries the same honest indicator ───────────────────────
  ...
  // The students screen lands on the LIVE view (#249) — the property under
  // test ("never a bare dot") now lives on its rows.
  await teacher.goto(`${BASE}/teacher/students`, { waitUntil: 'domcontentloaded' })
  await teacher.waitForSelector('.tch-liveRow', { timeout: 60000 })
  const rowDots = await teacher.locator('.tch-liveRow .tch-dot').count()
  const rowSeen = await teacher.locator('.tch-liveRow__seen').count()
  check('live rows show presence with a timestamp',
        rowDots > 0 && rowDots === rowSeen, `${rowDots} dots, ${rowSeen} labels`)
