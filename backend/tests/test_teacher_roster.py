...
class RosterIdentityUsesNames(unittest.IsolatedAsyncioTestCase):
    async def test_rows_do_not_carry_removed_profile_avatar_data(self):
        group_patch, member_patch, name_patch = _patches()
        with group_patch, member_patch, name_patch:
            roster = await teacher_roster.roster_for_teacher("teacher-a")
        self.assertTrue(roster["students"])
        self.assertTrue(all("avatar" not in row for row in roster["students"]))
