...
async def read_game_narration(game_id: str, learner_id: str = Depends(_reader)):
    ...
    # The plan pass writes its pitch on the game row (its own field, never
    # the kid's brief); serving it here too means a lost `plan` frame still
    # reaches the page on the next poll.
    if live is not None and game.get("pitch"):
        result["pitch"] = str(game.get("pitch"))[:600]
